  ...
  state.modal
    ...
    <section id="modal" onclick="app.run('hideModal')">
        <form onclick="event.stopPropagation();" onsubmit="event.stopPropagation();app.run('addUser', this);return false;">
            <input data-test-id="username-input" name="name" placeholder="username" required/>
            <input data-test-id="avatar-input" name="avatar" type="url" placeholder="avatar URL" required />
            <button data-test-id="add-user-button">Add User</button>
        </form>
    </section>
`
    : "";
...
const view = (state) =>
  ["<h1>Kanban</h1>", viewBoards(state), viewUsers(state), viewAddUserModal(state)].join("");
