const viewAddTaskModal = (state) =>
  state.modal
    ? `
    <section id="modal" onclick="app.run('hideModal')">
        <form data-test-id="add-task-modal" onclick="event.stopPropagation();" onsubmit="event.stopPropagation();app.run('addTask', this);return false;">
...
const viewTask = (task) => `
    <article id="${
      task.id
    }" class="task" draggable="true" ondragstart="app.run('ondragstart', event);">
        <p>${task.desc}</p>
